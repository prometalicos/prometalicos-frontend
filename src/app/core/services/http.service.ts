...
export class HttpService {
  ...
  private setHeader(name: string, value: string) {
    ...
    newopts.headers = newopts && newopts.headers && newopts.headers.set(name, value);
    return newopts;
  }
  ...
  private createOptions(opts?: Options): Options {
    ...
    if (opts) {
      ...
      if (opts.headers && !opts.headers.get('Content-Type')) {
        const defaultOpt = defaultOpts.headers && defaultOpts.headers.get('Content-Type') || '';
        opts.headers = opts.headers && opts.headers.set('Content-Type', defaultOpt)
        ;
      }
    }
